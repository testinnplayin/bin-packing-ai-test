...
describe('firstFit', () => {
  ...
  it('should pack items correctly when sorted in descending order', () => {
    ...
    // Expected packing: [[8, 2, 5], [8, 6], [9, 4], [9, 6], [9], [7]]
    const expectedNumberOfBins = 6;
    const expectedFirstBin = [8, 2, 5];
    const expectedSecondBin = [8, 6];
    const expectedThirdBin = [9, 4];
    const expectedFourthBin = [9, 6];
    const expectedFifthBin = [9];
    const expectedSixthBin = [7];
    // Sometimes the answer is not an optimized packing result like in this case so we expect less than 100% efficiency
    const expectedEfficiency = '81.11';

    const result = firstFit(items, binCapacity);

    const [firstBin, secondBin, thirdBin, fourthBin, fifthBin, sixthBin] = result.bins;
    expect(result.binCount).toBe(expectedNumberOfBins);
    expect(firstBin).toEqual(expectedFirstBin);
    expect(secondBin).toEqual(expectedSecondBin);
    expect(thirdBin).toEqual(expectedThirdBin);
